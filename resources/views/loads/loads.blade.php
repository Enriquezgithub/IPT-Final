...
@section('content')
    ...
    <div class="border shadow mb-5">
        <h2 class="ms-2 text-primary">Class</h2>
    </div>
    ...
    <div class="container border shadow rounded" style="width: 90%">
        ...
        <div class="row align-center">
            <p class="text-decoration-underline fw-bold mt-3 h5 text-success col-9">List of Class</p>
            <div class="col mt-3 ms-2">
              <input type="text" id="searchInput" placeholder="Search by ID" class="ms-4">
              <a href="{{url('/loads/create')}}" class=" btn bg-primary text-white">
            ...
            <table class="table table-bordered table-striped " id="studentTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$load->subject->description}}</td>
                        <td>{{$load->time}}
                            {{$load->day}}
                            {{$load->classroom->room_type}}
                        </td>
                        {{-- <td>{{$load->day}}</td> --}}
                        <td>{{$load->teacher->name}}</td>
                        <td class="text-center">
                                <a href="{{url('/loads/' . $load->id)}}">
